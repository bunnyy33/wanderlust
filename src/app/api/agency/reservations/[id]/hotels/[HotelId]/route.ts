...
interface ctx { params: Promise<{ id: string; HotelId: string }> }

export async function PUT(req: NextRequest, { params }: ctx) {
  if (!(await isAdminAuthed())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  try {
    const { id, HotelId } = await params;
    const existing = await db.hotelBooking.findFirst({ where: { id: HotelId, reservationId: id } });
    if (!existing) return NextResponse.json({ error: "Hotel booking not found" }, { status: 404 });
    ...
    if (body.checkInDate !== undefined) data.checkInDate = body.checkInDate ? new Date(body.checkInDate) : existing.checkInDate;
    if (body.checkOutDate !== undefined) data.checkOutDate = body.checkOutDate ? new Date(body.checkOutDate) : existing.checkOutDate;
    ...
    if (body.supplierId !== undefined) { data.supplierId = body.supplierId ? String(body.supplierId) : null; if (data.supplierId) { const sup = await db.supplier.findUnique({ where: { id: data.supplierId } }); data.supplierName = sup?.name ?? null; } else data.supplierName = null; }
    ...
    if (body.nights !== undefined) data.nights = Math.max(1, Number(body.nights));
    const merged = { ...existing, ...data };
    const nights = merged.nights > 0 ? merged.nights : calcNights(merged.checkInDate, merged.checkOutDate);
    data.nights = nights;
    const pricing = calcHotelPricing({ costPerNight: merged.costPerNight, sellPerNight: merged.sellPerNight, nights, noOfRooms: merged.noOfRooms });
    data.totalCost = pricing.totalCost;
    data.totalSell = pricing.totalSell;
    const updated = await db.hotelBooking.update({ where: { id: HotelId }, data });
    await recalcReservation(id);
    return NextResponse.json({ hotel: serializeHotelBooking(updated) });
  } catch (err) { console.error("agency hotel PUT error:", err); return NextResponse.json({ error: "Failed to update hotel" }, { status: 500 }); }
}
...
export async function DELETE(_req: NextRequest, { params }: ctx) {
  ...
  } catch (err) { console.error("agency hotel DELETE error:", err); return NextResponse.json({ error: "Failed to delete hotel" }, { status: 500 }); }
}
